...
class Dataset_HPI(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='MS', data_path='cropped_all_country.csv',
                 target='OT', scale=True, timeenc=0, freq='q', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
        self.args = args
        # info
        if size == None:
    ...
    def __read_data__(self):
        ...
        print('ATTEMPT PATH: ', os.path.join(self.root_path, self.data_path))
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        ...
        time_features = df_stamp[['year', 'quarter', 'month']]
        data_stamp = self.scaler.fit_transform(time_features)
